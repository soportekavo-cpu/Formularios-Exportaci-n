...
import { 
    ...
    UserIcon, ChevronDownIcon, LogOutIcon, LayoutDashboardIcon, Bars3Icon
} from './Icons';
        ...
        <div className="flex h-screen bg-background text-foreground overflow-hidden">
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 transform bg-card border-r border-border transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-auto lg:flex lg:flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <div className="flex items-center p-3 border-b border-border gap-2">
                    <button onClick={() => { setActiveCompany('dizano'); setViewingContractId(null); setIsMobileMenuOpen(false); }} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all border ${activeCompany === 'dizano' ? 'bg-primary text-primary-foreground border-primary' : 'bg-background text-muted-foreground hover:text-foreground border-border'}`}>DIZANO</button>
                    <button onClick={() => { setActiveCompany('proben'); setViewingContractId(null); setIsMobileMenuOpen(false); }} className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all border ${activeCompany === 'proben' ? 'bg-primary text-primary-foreground border-primary' : 'bg-background text-muted-foreground hover:text-foreground border-border'}`}>PROBEN</button>
                </div>
                <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                    {hasPermission('dashboard', 'view') && (
                        <button onClick={() => handleMobileNav(() => { setPage('dashboard'); setViewingContractId(null); })} className={`w-full flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors ${page === 'dashboard' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent/50 hover:text-foreground'}`}><LayoutDashboardIcon className="h-5 w-5 mr-3"/><span>Dashboard</span></button>
                    )}
                    
                    {hasPermission('contracts', 'view') && (
                        <button onClick={() => handleMobileNav(() => { setPage('shipments'); setViewingContractId(null); })} className={`w-full flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors ${page === 'shipments' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent/50 hover:text-foreground'}`}><ShipIcon className="h-5 w-5 mr-3"/><span>Contratos</span></button>
                    )}
                        ...
                        <>
                            ...
                            {menuItems.map(item => (
                                hasPermission(item.resource, 'view') && (
                                    <button 
                                        key={item.type}
                                        onClick={() => handleMobileNav(() => { setActiveCertType(item.type as CertificateType); setView('list'); setPage('documents'); setViewingContractId(null); })}
                    ...
                    {hasPermission('admin', 'view') && (
                        <button onClick={() => handleMobileNav(() => { setPage('admin'); setViewingContractId(null); })} className={`w-full flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors ${page === 'admin' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent/50 hover:text-foreground'}`}><CogIcon className="h-5 w-5 mr-3"/><span>Configuración</span></button>
                    )}
                </nav>
            </aside>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                {/* Header */}
                <header className="h-16 flex-shrink-0 border-b border-border flex items-center justify-between px-4 lg:px-6 bg-card/30 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        {/* Mobile Menu Button */}
                        <button 
                            className="lg:hidden p-2 -ml-2 rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"
                        ...
                        {page === 'shipments' && !viewingContractId && hasPermission('contracts', 'create') && (
                            <button
                                ...
                                className="inline-flex items-center gap-x-2 rounded-full bg-primary px-3 py-1.5 text-xs sm:text-sm font-semibold text-primary-foreground shadow-sm hover:bg-primary/90 transition-all active:scale-95"
                            >
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <div className="relative" ref={alertMenuRef}>
                            ...
                            {isAlertMenuOpen && (
                                <div className="absolute right-0 mt-2 w-72 sm:w-80 origin-top-right rounded-lg bg-popover shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border border-border z-20 overflow-hidden">
                        ...
                        <div className="relative" ref={userMenuRef}>
                            <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="flex items-center gap-x-2 p-1.5 sm:pr-3 rounded-full hover:bg-accent transition-colors border border-transparent hover:border-border">
                                ...
                                <ChevronDownIcon className={`w-4 h-4 text-muted-foreground transition-transform hidden sm:block ${isUserMenuOpen ? 'rotate-180' : ''}`} />
                            </button>
                            {isUserMenuOpen && (
                                <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-popover shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border border-border z-10">
                                    <div className="py-3 px-4 border-b border-border mb-1">
                                        <p className="text-sm font-semibold text-foreground truncate">{currentUser.name}</p>
                                        <p className="text-xs text-muted-foreground truncate">{currentUser.email}</p>
                                    </div>
                                    <div className="py-1">
                                        <button onClick={onLogout} className="w-full text-left flex items-center gap-x-2 text-red-600 dark:text-red-400 px-4 py-2 text-sm hover:bg-accent transition-colors">
