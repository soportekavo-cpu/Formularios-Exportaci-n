...
const NotifierManager: React.FC<NotifierManagerProps> = ({ notifiers, setNotifiers }) => {
  ...
  const openModal = (item: Partial<Notifier> | null = null) => {
  ...
  return (
    <div className="bg-card p-4 rounded-lg border">
                        ...
                        <div className="bg-card px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            <div className="sm:flex sm:items-start">
                                ...
                                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    <h3 className="text-base font-semibold leading-6 text-foreground" id="modal-title">Eliminar Notificador</h3>
                                    <div className="mt-2">
                                        <p className="text-sm text-muted-foreground">¿Estás seguro de que deseas eliminar este notificador? Esta acción no se puede deshacer.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="flex min-h-full items-center justify-center p-4">
              <div className="relative transform overflow-hidden rounded-lg bg-card text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border">
                <div className="bg-card px-6 pt-5 pb-4">
                  <h3 className="text-lg font-semibold leading-6 text-foreground">{editingItem?.id ? 'Editar' : 'Agregar'} Notificador</h3>
      ...
      <div className="flex items-center justify-between gap-4 mb-4">
        <div>
          <h3 className="font-semibold text-foreground">Notificadores</h3>
          <p className="text-sm text-muted-foreground">Gestiona la información de los notificadores.</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-border">
         ...
         {notifiers.length === 0 && <p className="text-center py-6 text-sm text-muted-foreground">No hay notificadores guardados.</p>}
      </div>
    </div>
  );
};
