...
const workflowStepsList = [
    { id: 'informe_venta', label: 'Informe de Venta (Anacafé)' },
    { id: 'contrato_fob', label: 'Contrato FOB' },
    { id: 'factura_especial', label: 'Factura Especial (Si aplica)' },
    { id: 'pago_impuestos', label: 'Pago de Impuestos' },
    { id: 'confirmacion_zarpe', label: 'Confirmación de Zarpe' },
    { id: 'documentos_finales', label: 'Documentos Finales (OIC, BL, etc.)' }
];
...
const PartidaModal: React.FC<PartidaModalProps> = ({ isOpen, onClose, onSave, initialData, contractId, contractDifferential, activeCompany, isReadOnly = false, contracts = [] }) => {
  ...
  const handleWorkflowChange = (stepId: string, isCompleted: boolean) => {
    if (isReadOnly) return;
    setPartida(prev => {
        const currentWorkflow = prev.workflow || [];
        const existingStepIndex = currentWorkflow.findIndex(s => s.id === stepId);
        let newWorkflow = [...currentWorkflow];

        if (existingStepIndex >= 0) {
            newWorkflow[existingStepIndex] = { 
                ...newWorkflow[existingStepIndex], 
            ...
            newWorkflow.push({
                id: stepId,
                label: workflowStepsList.find(s => s.id === stepId)?.label || '',
                status: isCompleted ? 'completed' : 'pending',
                completedDate: isCompleted ? new Date().toISOString().split('T')[0] : undefined
            });
        }
  ...
  const getStepStatus = (stepId: string) => {
      return partida.workflow?.find(s => s.id === stepId)?.status === 'completed';
  };
              ...
              <div className="space-y-6 max-h-[75vh] overflow-y-auto pr-3">
                 ...
                 <div className="bg-indigo-50/50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div className="col-span-1 md:col-span-3 flex gap-8 items-center py-2 px-4 bg-white dark:bg-background rounded-md border border-indigo-100 dark:border-indigo-800">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-indigo-100 dark:border-indigo-800">
                 ...
                 <div className="bg-green-50/50 dark:bg-green-900/10 border border-green-100 dark:border-green-800 rounded-lg p-4">
                     <h4 className="text-sm font-bold text-green-700 dark:text-green-400 mb-3 uppercase tracking-wide border-b border-green-200 dark:border-green-800 pb-1">Permisos y Workflow</h4>
                     ...
                     <div className="grid grid-cols-2 md:grid-cols-3 gap-3 border-t border-green-200 dark:border-green-800 pt-4">
                        {workflowStepsList.map((step) => (
                            <label key={step.id} className={`flex items-center p-2 rounded border transition-colors ${getStepStatus(step.id) ? 'bg-green-100 border-green-300' : 'bg-white dark:bg-background border-gray-200'} ${isReadOnly ? 'cursor-default' : 'cursor-pointer hover:bg-green-50'}`}>
                                <input 
                                    type="checkbox" 
                                    checked={getStepStatus(step.id)} 
                                    onChange={(e) => handleWorkflowChange(step.id, e.target.checked)} 
                                    className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded mr-2"
                                    disabled={isReadOnly}
                                />
                                <span className={`text-xs font-medium ${getStepStatus(step.id) ? 'text-green-800' : 'text-gray-600'}`}>{step.label}</span>
                            </label>
                        ))}
                     </div>
                 </div>
